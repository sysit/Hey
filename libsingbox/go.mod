module hey/libsingbox

// sing-box v1.11+ 需要 Go 1.23+。构建脚本会 `go mod tidy` 拉齐其余依赖
// （sing-box / sing-tun / sagernet 的 gvisor fork 等）。
go 1.23.0

// 钉一个已知可用的 sing-box tag。spike 期可按需 bump，但务必锁死，
// 因为 libbox.PlatformInterface 的方法集会随版本变动（见 platform_stub.go）。
require github.com/sagernet/sing-box v1.11.0
