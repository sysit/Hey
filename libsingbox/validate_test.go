//go:build with_utls && with_clash_api && with_gvisor && with_quic

package main

// 用真实 sing-box v1.12.x 校验 SingboxConfig.ets 生成器产出的配置 schema。
//
...
// 缺 with_quic 时 tuic/hysteria2 出站不注册（"QUIC is not included in this build"）——
// 这正是本测试抓出的构建漏洞。在本机跑：
//   cd libsingbox && go test -tags "with_gvisor with_utls with_clash_api with_quic" -run TestGeneratedConfigs -v
...
func TestGeneratedConfigs(t *testing.T) {
	...
	// sing-box 独有协议（Xray 无对应 outbound）。anytls 依赖 sing-box 1.12+，
	// 是这批用例的重点：它在 1.11 会被 "unknown outbound type: anytls" 拒。
	validateConfig(t, "anytls", anytlsTls)
	validateConfig(t, "tuic", tuicTls)
	validateConfig(t, "hysteria2", hysteria2Obfs)
}
...
// 域名服务器 → 触发 dns.rules 的 direct-dns 解析分支（IP 服务器则无此规则）。
// anytls 的 TLS 字段内联在 server 上，由 buildServerTls 组装（含 utls 指纹）。
const anytlsTls = `{
 "log":{"level":"warn","timestamp":true},
 "dns":{"servers":[{"tag":"proxy-dns","address":"1.1.1.1","detour":"proxy"},{"tag":"direct-dns","address":"223.5.5.5","detour":"direct"}],"rules":[{"domain":["anytls.example.com"],"server":"direct-dns"}],"final":"proxy-dns","strategy":"ipv4_only"},
 "inbounds":[{"type":"mixed","tag":"socks-in","listen":"127.0.0.1","listen_port":10810,"sniff":true,"sniff_override_destination":false}],
 "outbounds":[
   {"type":"anytls","tag":"proxy","server":"anytls.example.com","server_port":8443,"password":"anytls-pass","tls":{"enabled":true,"server_name":"anytls.example.com","utls":{"enabled":true,"fingerprint":"chrome"}}},
   {"type":"direct","tag":"direct"},
   {"type":"dns","tag":"dns-out"}
 ],
 "route":{"rules":[{"protocol":"dns","outbound":"dns-out"},{"ip_is_private":true,"outbound":"direct"}],"final":"proxy","auto_detect_interface":false}
...
const tuicTls = `{
 "log":{"level":"warn","timestamp":true},
 "dns":{"servers":[{"tag":"proxy-dns","address":"1.1.1.1","detour":"proxy"},{"tag":"direct-dns","address":"223.5.5.5","detour":"direct"}],"rules":[{"domain":["tuic.example.com"],"server":"direct-dns"}],"final":"proxy-dns","strategy":"ipv4_only"},
 "inbounds":[{"type":"mixed","tag":"socks-in","listen":"127.0.0.1","listen_port":10810,"sniff":true,"sniff_override_destination":false}],
 "outbounds":[
   {"type":"tuic","tag":"proxy","server":"tuic.example.com","server_port":443,"uuid":"b831381d-6324-4d53-ad4f-8cda48b30811","password":"tuic-pass","tls":{"enabled":true,"server_name":"tuic.example.com","alpn":["h3"]},"congestion_control":"bbr","udp_relay_mode":"native"},
   {"type":"direct","tag":"direct"},
   {"type":"dns","tag":"dns-out"}
 ],
 "route":{"rules":[{"protocol":"dns","outbound":"dns-out"},{"ip_is_private":true,"outbound":"direct"}],"final":"proxy","auto_detect_interface":false}
...
// Hysteria2：恒 TLS，缺省 alpn 走 h3；带 salamander 混淆与上下行带宽。
const hysteria2Obfs = `{
 "log":{"level":"warn","timestamp":true},
 "dns":{"servers":[{"tag":"proxy-dns","address":"1.1.1.1","detour":"proxy"},{"tag":"direct-dns","address":"223.5.5.5","detour":"direct"}],"rules":[{"domain":["hy2.example.com"],"server":"direct-dns"}],"final":"proxy-dns","strategy":"ipv4_only"},
 "inbounds":[{"type":"mixed","tag":"socks-in","listen":"127.0.0.1","listen_port":10810,"sniff":true,"sniff_override_destination":false}],
 "outbounds":[
   {"type":"hysteria2","tag":"proxy","server":"hy2.example.com","server_port":443,"password":"hy2-pass","tls":{"enabled":true,"server_name":"hy2.example.com","alpn":["h3"]},"obfs":{"type":"salamander","password":"obfs-pass"},"up_mbps":100,"down_mbps":200},
   {"type":"direct","tag":"direct"},
   {"type":"dns","tag":"dns-out"}
 ],
 "route":{"rules":[{"protocol":"dns","outbound":"dns-out"},{"ip_is_private":true,"outbound":"direct"}],"final":"proxy","auto_detect_interface":false}
