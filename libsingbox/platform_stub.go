...
// LocalDNSTransport：sing-box 1.12 新增。返回 nil 表示不提供平台托管的 DNS 传输，
// 内核改用配置里的 DNS（libbox/config.go 对 nil 有判空，安全）。
func (p *platformInterface) LocalDNSTransport() libbox.LocalDNSTransport { return nil }

// SystemCertificates：sing-box 1.12 新增。返回 nil 表示不追加系统证书
// （libbox 的 iteratorToArray 对 nil 迭代器安全，返回空数组）。
func (p *platformInterface) SystemCertificates() libbox.StringIterator { return nil }
...
// PlatformInterface 没有 Use*Monitor/Getter 开关 —— 宿主总是要提供接口监控。
// 给空实现：固定配置下不依赖接口更新事件。若设备上因缺接口信息出问题，
