...
#     启动即 "QUIC is not included in this build, rebuild with -tags with_quic"。
#     （anytls 不依赖 QUIC，但需 sing-box 1.12+ 才有该出站类型。）
# ⚠️ 不能加 netgo：openharmony 的 net 端口需要 cgo，加了会报 _C_getifaddrs undefined。
GO_TAGS="${GO_TAGS:-with_gvisor with_utls with_clash_api with_quic}"
