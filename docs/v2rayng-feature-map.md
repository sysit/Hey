...
| Xray config generation | Present for outbound + native TUN inbound + metrics inbound + optional local HTTP proxy inbound/listen sharing + basic direct/block outbounds. Full custom Xray configs are validated and passed through unchanged at runtime. |
