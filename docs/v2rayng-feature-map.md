...
| Persistent app settings | Present for core VPN, v2rayNG `pref_mode` (`VPN` / `Proxy only`), DNS, DNS hosts, IPv6 enable/prefer split, mux/XUDP/fragment, sniffing, log, speed display, current connection info test URL, UI mode, all-groups display, double-column display, real-ping concurrency, delete confirmation, immediate scan startup, and routing strategy; selected values are applied at app launch/foreground, save time, connection start, batch test time, Scanner startup, destructive actions, or node list rendering, including Harmony color mode, all-groups node aggregation, double-column node list lanes, IP API lookup via the generated local HTTP proxy, VPN interface DNS, generated Xray `dns.hosts`, VPN IPv6 routing, WireGuard IPv6 local address filtering, outbound Happy Eyeballs, mux settings, outbound fragment settings, speed metrics generation/display, delay-test concurrency, scan auto-launch, deletion confirmation, and native proxy-only start/stop without VPN authorization. |
...
| Multi-subscription groups | Present with legacy single-subscription migration. Rename/edit, enable-disable, delete, delete confirmation, reorder, batch update all, per-subscription insecure URL opt-in, auto-update opt-in, and update interval are wired. Foreground due refresh is wired, and subscription fetches can prefer the local HTTP proxy when the VPN runtime exposes it. Background refresh remains pending. |
