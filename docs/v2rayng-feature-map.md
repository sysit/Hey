...
| SettingsActivity | Config | Core, VPN DNS, IPv6 preference, local HTTP proxy append, mux, sniffing, log level and routing strategy persist; VPN DNS feeds Harmony `VpnConfig.dnsAddresses`, IPv6 preference feeds the VPN interface address/route, while remote/domestic DNS feed generated Xray DNS. Dedicated pickers and full advanced options pending. |
...
| Persistent app settings | Present for core VPN, DNS, IPv6, mux, sniffing, log and routing strategy; selected values are applied at connection start, including VPN interface DNS and IPv6 routing. |
