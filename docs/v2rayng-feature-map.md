...
| SubSettingActivity / SubEditActivity | Subs | Multi-group model present. Save/update/update-all/select/enable-disable/delete/reorder are wired; dedicated edit form supports name, URL, User-Agent, node-name filter, subscription-level insecure URL opt-in, auto-update opt-in, and update interval. Foreground due refresh is wired, and Harmony WorkScheduler background refresh is code-complete pending real-device wakeup regression. |
| ServerActivity protocol editors | Add | Partial. Field-level editor is present for VLESS/VMess/Trojan/Shadowsocks/SOCKS/HTTP/WireGuard/Hysteria2 and saves validated outbound JSON. Reality fields include `spiderX` and `mldsa65Verify`/`pqv` for VLESS/Trojan. TUIC is not targeted for Xray-core runtime because v2rayNG keeps it disabled and current Xray-core config docs do not expose a TUIC outbound. |
...
| SettingsActivity | Config | Core, VPN DNS, IPv6 enable, IPv6 preference, local HTTP proxy append/sharing, local SOCKS proxy static/dynamic port/UDP/auth, mux, sniffing, log level, speed display, current connection info test URL, UI mode, all-groups display, double-column display, real-ping concurrency, delete confirmation, immediate scan startup, auto connect at startup, and routing strategy persist; UI mode maps v2rayNG `0/1/2` to Harmony follow-system/light/dark color mode, all-groups display adds an All tab that aggregates nodes from every subscription group for search, delay testing, and export, double-column display switches the Nodes list between one and two lanes, the IP API URL defaults to v2rayNG's `https://api.ip.sb/geoip` and is queried through the local HTTP proxy after a successful start, VPN DNS feeds Harmony `VpnConfig.dnsAddresses`, `ipv6Enabled` feeds the VPN interface IPv6 address/route and WireGuard IPv6 local address retention, `preferIpv6` feeds generated Xray outbound Happy Eyeballs, proxy sharing changes local HTTP/SOCKS inbound listen, dynamic SOCKS port resolves a free runtime port before start, remote/domestic DNS feed generated Xray DNS, DNS hosts accepts v2rayNG `domain:address,...` format into `dns.hosts`, mux/XUDP/fragment advanced fields feed outbound runtime settings, speed display controls generated metrics/stats/policy, runtime traffic display, and an ongoing Harmony speed notification refreshed every 3 seconds while connected, real-ping concurrency controls batch delay testing, delete confirmation controls node/subscription removal dialogs, immediate scan startup controls Scanner page auto-launch, auto connect at startup handles Harmony `AUTO_STARTUP` launches when the system/user permits app auto-start, and log level now uses a dedicated picker. Remaining gaps are deeper platform-specific settings and full device regression. |
...
| TaskerActivity / shortcuts / widgets / QS tile | Platform equivalents | Partial. Harmony control deep links now cover the core start/stop/toggle/scan actions (`hey://control?action=...` plus short hosts) and can be invoked by external Want/automation surfaces. Native desktop service cards/widgets remain pending. |
...
| Persistent app settings | Present for core VPN, v2rayNG `pref_mode` (`VPN` / `Proxy only`), DNS, DNS hosts, IPv6 enable/prefer split, mux/XUDP/fragment, sniffing, log, speed display, current connection info test URL, UI mode, all-groups display, double-column display, real-ping concurrency, delete confirmation, immediate scan startup, auto connect at startup, and routing strategy; selected values are applied at app launch/foreground, save time, connection start, batch test time, Scanner startup, destructive actions, node list rendering, Harmony auto-start launch handling, or runtime notification sync, including Harmony color mode, all-groups node aggregation, double-column node list lanes, IP API lookup via the generated local HTTP proxy, VPN interface DNS, generated Xray `dns.hosts`, VPN IPv6 routing, WireGuard IPv6 local address filtering, outbound Happy Eyeballs, mux settings, outbound fragment settings, speed metrics generation/display/ongoing notification, delay-test concurrency, scan auto-launch, deletion confirmation, startup auto-connect when system auto-start is granted, and native proxy-only start/stop without VPN authorization. |
...
| SOCKS/HTTP/WireGuard/Hysteria2 parsing | Present for share-link import, subscription discovery, and manual outbound editing; WireGuard `[Interface]/[Peer]` `.conf` text/file import is also supported. TLS/Reality `ech`/`pcs` and Reality `spx`/`pqv` parameters round-trip through share-link import/export, TLS/Reality `ech`/`pcs` can be filled from NodeEdit manual forms, and TLS insecure aliases (`insecure`/`allowInsecure`/`allow_insecure`) are accepted on import. Runtime connection for WireGuard/Hysteria2 still needs core validation. |
...
| Delay test / real ping / sort by delay | Present. Per-node real outbound delay via libXray `CGoPing` (own SOCKS test inbound, preferring `CGoGetFreePorts` dynamic ports with 10825 fallback), persisted per node, with sort-by-delay. Batch delay testing honors the v2rayNG-style concurrency setting (default 16, clamped to 1..128). Falls back to direct URL test when the native core is unavailable (e.g. emulator). Needs real-device validation. |
...
| Multi-subscription groups | Present with legacy single-subscription migration. Rename/edit, enable-disable, delete, delete confirmation, reorder, batch update all, per-subscription insecure URL opt-in, auto-update opt-in, and update interval are wired. Foreground due refresh is wired, subscription fetches can prefer the local HTTP proxy when the VPN runtime exposes it, and WorkScheduler background refresh is code-complete pending real-device wakeup regression. |
