...
| SOCKS/HTTP/WireGuard/Hysteria2 parsing | Present for share-link import, subscription discovery, and manual outbound editing; WireGuard `[Interface]/[Peer]` `.conf` text/file import is also supported. Reality `spx`/`pqv` parameters round-trip through share-link import/export and Clash.Meta subscriptions. Runtime connection for WireGuard/Hysteria2 still needs core validation. |
