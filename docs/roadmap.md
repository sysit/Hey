...
| Geo 资产管理 | ✅ 98% | 下载 / 自定义 URL / 本地 `.dat` 导入 / 资源 URL 二维码导入 / 自定义资源名称唯一性校验 / 本地文件资源隐藏编辑入口（按 `url == "file"` 行为）/ 内置下载已包含 v2rayNG 强制更新的 `geoip-only-cn-private.dat`，用于支撑 `geoip:cn/private` ext 路由，下载源选择补齐 v2rayNG 的 Loyalsoldier / Russia / Iran 三组规则源；Geo 文件 native 计数/校验已接线，`libxray.so` 已重建并导出 Geo 符号，待真机验证 |
...
| 2026-06-20 | 阶段 5 | 🟡 备份能力暂缓（无云服务器）：WebDAV 与剪贴板备份均下线，优先保留资源下载与恢复主链路 |
