...
| 节点延迟测速 / 排序 | ✅ 90% | 节点菜单同时提供 TCP 延迟、v2rayNG 风格真连接测速和手动按测试结果排序；真连接测速走 `CGoPing` + 独立 SOCKS 测试 inbound，优先使用 `CGoGetFreePorts` 动态端口，测速配置会按 v2rayNG 移除 outbound mux，并按 `delayTestUrl` 设置访问目标 URL；普通 outbound 启动成功后会追加 v2rayNG 单次连接延迟日志，主 URL 失败时重试备用 `https://www.google.com/generate_204`，成功后再查询出口 IP；批量测速并发数已按 v2rayNG 设置接线，手动/自动排序与删除失败节点均按订阅分组与 All 虚拟分组语义落盘；高级/完整配置的运行态 native core 级测速仍需后续补齐与真机验证 |
