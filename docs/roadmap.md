...
| 路由设置页 | ✅ 83% | 广告拦截、自定义规则、预设规则集导入/导出均已生效；路由域名策略按 v2rayNG `AsIs/IPIfNonMatch/IPOnDemand` 三值归一化后保存并写入 Xray；`routeOnly` 控制 process 规则输出和 sniffing routeOnly；自定义规则可选择当前高级出站目标；真机规则回归待补 |
