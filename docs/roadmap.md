...
| 节点延迟测速 / 排序 | ✅ 88% | 节点菜单同时提供 TCP 延迟和 v2rayNG 风格真连接测速；真连接测速走 `CGoPing` + 独立 SOCKS 测试 inbound，优先使用 `CGoGetFreePorts` 动态端口，并按 `delayTestUrl` 设置访问目标 URL；批量测速并发数已按 v2rayNG 设置接线，测速后自动排序/删除失败节点已按订阅分组与 All 虚拟分组语义落盘；需真机验证 |
