...
- ✅ 相机扫码与相册图片二维码导入：`pages/Scanner.ets` 已接入 `@kit.ScanKit`（`scanBarcode.startScanForResult` / `detectBarcode.decode`），共享页头会随异步语言状态刷新，深链进入扫码页时标题与内容语言一致；首页统计标签、无订阅引导空态、节点页当前分组空态、添加节点页主入口、JSON 导入页、手动节点编辑页、路由设置页与 Assets 页可见说明也会随当前语言刷新，避免英文界面残留中文文案
...
- ✅ Assets 页模块化、NodeEdit 组件化、8 协议手动配置与解析；内置 Geo 下载已包含 v2rayNG 强制更新的 `geoip-only-cn-private.dat`，下载源包含 v2rayNG 的 Loyalsoldier / Russia / Iran 三组规则源，资源下载已按 v2rayNG 优先经本地 HTTP 代理并直连兜底；Assets 页主视图、用户资源列表和相关 toast 已跟随当前语言，NodeEdit 标题、分区、字段、保存按钮和校验提示已跟随当前语言
...
| 备份功能 | 不进入当前复刻范围；当前版本已移除本地 ZIP、剪贴板备份/还原和 WebDAV 传输实现（无云服务器时先不上线） | 仅在后续有稳定云端目标时重新立项 |
...
- 配置：启用规则按列表顺序写入 `routing.rules`，位置在广告拦截之后、`routeOnly` 绕过规则之前；未知 outboundTag 回退 `proxy`；`routeOnly` 开关会按 v2rayNG 语义写入 TUN sniffing `routeOnly`，process 规则输出还会按 v2rayNG `canUseProcessRouting` 要求 Hev TUN 关闭；运行配置生成时将路由 IP 匹配里的 `geoip:cn/private` 改写为 `ext:geoip-only-cn-private.dat:cn/private`，并在可用时将 process 包名解析为 Harmony app UID（未知应用哨兵 `__unknown_app__` → `-1`）
...
**进展（2026-06-18 续，2026-06-20 已升级）**：订阅 WorkScheduler 后台调度已接入：
- 调度：按已开启自动更新且 URL 有效的订阅分组生成稳定 workId 与独立 repeat interval，注册每订阅一个持久重复 `workScheduler` 任务
...
**进展（2026-06-20 续）**：订阅列表页本地化已落地：
- UI：订阅分组页摘要、空态提示、本地节点数量和“从未更新”状态均从 i18n 字典读取，英文单数摘要使用 `1 subscription group`
- Controller：本地分组跳过更新、批量更新全部、前台到期自动更新和单订阅后台更新结果 toast 均跟随当前语言
- 验证：模拟器英文环境从 Settings 打开 Subscription groups，布局中可见 `Subscription groups` 和 `1 subscription group`，且无旧中文摘要残留
...
- 🟡 **备份能力剔除当前范围**：当前不做本地/云端备份，已删除 ZIP 备份、剪贴板备份/还原与 WebDAV 传输残留；待有稳定云端服务后再另起功能闭环评估 WebDAV（或自定义后端）

**v2rayNG 对照**：`BalancerStrategyType`、`ServerGroupActivity`、`ServerProxyChainActivity`、
`WebDavManager`（备份仅作上游参考，Hey 当前不纳入复刻验收）。
**验收标准**：策略组按延迟自动择优；备份能力不作为当前验收项。
**依赖**：M2（路由系统）、M3（订阅）。
...
| 2026-06-20 | M4 | ✅ Geo/自定义资源下载代理兜底完成（本地 HTTP 代理暴露时先经 `127.0.0.1:10809` 请求，失败后自动直连；未暴露代理时保持直连；补代理端口选择单测） |
...
| 2026-06-20 | M2 | ✅ process routing Hev 门槛对齐 v2rayNG（`canUseProcessRouting` 要求 `routeOnly` 开启且 Hev TUN 关闭；普通自定义规则与完整 custom config 的 `process` UID 转换共用该门槛；补配置生成/归一化单测） |
...
| 2026-06-18 | M3 | 🟡 订阅 WorkScheduler 后台调度接线（持久重复任务 + `SubscriptionUpdateWorkAbility` 到期刷新 + 增删改/首页同步注册 + 诊断日志 + 调度计划单测）；当时为统一最短间隔任务，待真机触发回归 |
| 2026-06-20 | M3 | ✅ 订阅后台 per-subscription 调度完成（按 v2rayNG `SubscriptionUpdater` 为每个自动更新订阅生成稳定 workId 与独立 repeat interval，WorkScheduler 参数携带 groupId，后台任务只刷新目标订阅；补计划单测） |
...
| 2026-06-20 | M5 | 🟡 备份能力剔除当前范围（无云服务器）：删除 ZIP/WebDAV/剪贴板备份残留实现，保留配置与资源功能的主链路；后续有稳定云端目标再重新立项 |
...
| 2026-06-20 | M4 | ✅ 添加节点页主入口本地化完成（链接/扫码导入、手动协议列表、高级配置、JSON 配置入口和导入结果 toast 均走 i18n；模拟器英文布局验证 `Scan & link import`、`Create proxy chain` 且无旧中文入口残留） |
...
| 2026-06-20 | M4 | ✅ 手动节点编辑页本地化补齐（NodeEdit 标题、分区、基础/认证/传输/TLS/Reality/WG/HY2 字段、保存按钮和校验/保存提示均走 i18n；字段行改为 key 内部翻译以修复异步语言加载后不刷新；模拟器英文 VLESS 页验证 `Node name`、`Save node config` 且无旧中文字段残留） |
...
| 2026-06-20 | M3 | ✅ 回环/内网 HTTP 订阅链接放行对齐 v2rayNG（新增 `core/PrivateNetwork.ets` 复刻 v2rayNG `Utils.isIpInCidr`（IPv4 32 位掩码）与 `AppConfig.PRIVATE_IP_LIST`，`SubscriptionManager.validateSubscriptionUrl` 按 v2rayNG `isValidSubUrl` 对回环/内网 IP host 的明文 HTTP 订阅免“允许不安全”开关放行，公网明文 HTTP 仍拦截；补 `isIpInCidr` UtilsTest 对照与回环/内网/公网 URL 单测，主包编译通过） |
| 2026-06-20 | 自查 | ✅ 分享链接协议源码级对照完成（逐行核对 v2rayNG `FmtBase`/`VmessFmt`/`ShadowsocksFmt`/`SocksFmt`/`Hysteria2Fmt`：共享 url-style 解析/导出（security 仅 tls/reality 保留、3 键 allowInsecure、h2/http→type=http、insecure 仅 TLS 写、reality 全参数）、VMess QR gRPC/KCP 字段复用、SOCKS 明文/base64 userInfo、Hysteria2 `mportHopInt` 区间→中点规范化全部一致；仅 VMess QR 空 `net`、SOCKS 单值 userInfo 等边界处 Hey 更宽松，属可接受偏差不改；新增 Hysteria2 `mportHopInt` 导出规范化（10-60→35、1-3→5、30→30、3 丢弃）回归测试） |
