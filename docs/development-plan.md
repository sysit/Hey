...
| 订阅 | 多分组 + 手动/批量更新 + 前台到期刷新 + 本地 HTTP 代理经由更新 + WorkScheduler 后台调度接线；订阅请求保留自定义 User-Agent，并按 v2rayNG 支持 URL 内嵌 `user:pass@host` Basic Auth；本地 SOCKS 入口按 v2rayNG 默认开启，可供代理经由能力使用 | 待真机触发回归后台唤醒路径 |
