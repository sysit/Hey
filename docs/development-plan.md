...
- [x] 真连接延迟测试：Settings 保存 `realPingConcurrency` 与 `delayTestUrl`；节点菜单提供 TCP 延迟和真连接测速两个入口，真连接批量测速按配置分批并发、通过 native `CGoPing` 访问目标 URL 并串行落盘；普通 outbound 启动成功后追加 v2rayNG 单次连接延迟日志，主 URL 失败会重试备用 `https://www.google.com/generate_204`，成功后再查出口 IP（2026-06-20 补运行态备用 URL）
